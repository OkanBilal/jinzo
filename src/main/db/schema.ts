...
export const feedItems = sqliteTable(
  ...
  (t) => [
    ...
    check("check_metadata_json", sql`json_valid(${t.metadata}) OR ${t.metadata} IS NULL`),
  ]
);
...
export const connections = sqliteTable(
  ...
  (t) => [
    ...
    check("check_metadata_json", sql`json_valid(${t.metadata}) OR ${t.metadata} IS NULL`),
    check("check_scopes_json", sql`json_valid(${t.scopes}) OR ${t.scopes} IS NULL`),
  ]
);
...
export const connectionSyncState = sqliteTable(
  ...
  (t) => [
    ...
    check("check_cursor_json", sql`json_valid(${t.cursor}) OR ${t.cursor} IS NULL`),
  ]
);
...
export const appStates = sqliteTable(
  ...
  (t) => [
    ...
    check("check_enabled_features_json", sql`json_valid(${t.enabledFeatures}) OR ${t.enabledFeatures} IS NULL`),
    check("check_config_json", sql`json_valid(${t.config}) OR ${t.config} IS NULL`),
  ]
);
