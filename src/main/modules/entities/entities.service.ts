...
export const entitiesService = {
  ...
  // Playlist Operations
  // ─────────────────────────────────────────────────────────────
  async getPlaylistItems(playlistEntityId: string): Promise<ServiceResponse<unknown[]>> {
    try {
      const items = await entitiesRepo.findPlaylistItems(playlistEntityId);
      return { success: true, data: items };
    } catch (error) {
  ...
  async addPlaylistItem(
    ...
    try {
      await entitiesRepo.addPlaylistItem(playlistEntityId, itemEntityId, position);
      return { success: true };
    } catch (error) {
  ...
  async removePlaylistItem(
    ...
    try {
      await entitiesRepo.removePlaylistItem(playlistEntityId, itemEntityId);
      return { success: true };
    } catch (error) {
  ...
  async reorderPlaylistItem(
    ...
    try {
      await entitiesRepo.reorderPlaylistItem(playlistEntityId, itemEntityId, newPosition);
      return { success: true };
    } catch (error) {
