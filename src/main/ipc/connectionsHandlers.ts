...
import { connections, connectionTokens, connectionResources, appStates } from "../db/schema";
...
/**
 * Register all IPC handlers for connections operations
 */
export function registerConnectionsHandlers() {
  // Fetch GitHub repositories
  ipcMain.handle("connections:getGithubRepos", async (_, connectionId: string) => {
    try {
      const db = getDb();
  ...
  ipcMain.handle("connections:getRaindropCollections", async (_, connectionId: string) => {
    try {
      const db = getDb();
      ...
      const formattedCollections = data.items.map((collection: any) => ({
        id: collection._id,
  ...
  ipcMain.handle("connections:toggleHackerNews", async (_, payload: {
    enabled: boolean;
    username?: string;
    topStories?: boolean;
  ...
  ipcMain.handle("connections:saveResources", async (_, payload: {
    provider: string;
    connectionId: string;
    resources?: any[];
    sources?: string[];
  ...
  ipcMain.handle("connections:removeResource", async (_, resourceId: string) => {
    try {
      const db = getDb();
  ...
  ipcMain.handle("connections:getSelectedResources", async (_, provider: string) => {
    try {
      const db = getDb();
