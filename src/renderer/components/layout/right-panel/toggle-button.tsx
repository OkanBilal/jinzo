...
export function ToggleButton({ isOpen, onClick }: ToggleButtonProps) {
  return (
    <Button
      ...
      onClick={onClick}
      className="fixed z-(--z-panel-toggle) rounded-full! p-1.25! text-primary-700 dark:text-primary-300! bg-primary-100/40 dark:bg-primary/5 transition-all duration-300 ease-out top-2.75 right-3.25"
      aria-label={isOpen ? "Close right panel" : "Open right panel"}
    >
      {isOpen ? (
        <Close className="size-3.75" />
      ) : (
        <RightPanelOpen className="size-3.75 rotate-180" />
      )}
    </Button>
  );
}
