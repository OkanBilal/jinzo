...
export default function CursorPage() {
  const ws = useWorkspacePage(CURSOR_PROVIDER_ID);
  const [abortRun] = useAbortRunMutation();
  const { data: providerData } = useGetProviderByIdQuery(CURSOR_PROVIDER_ID);
  ...
  const handleApplyPlan = useCallback(async () => {
    if (providerData) {
      ...
      if ((currentConfig as any).mode === "plan") {
        await updateProvider({
          id: CURSOR_PROVIDER_ID,
          payload: { config: { ...currentConfig, mode: "agent" } },
        });
      }
    }
