...
- Routes: `/` (default route), `/settings`, `/copilot[/:workspaceId]` (GitHub Copilot agent), `/Codex[/:workspaceId]` (Codex agent), `/codex[/:workspaceId]` (OpenAI Codex agent), `/cursor[/:workspaceId]` (Cursor agent), `/plugins`, `/pulse`, `/relay`
- Copilot / Codex / Codex / Cursor routes share the same workspace UI but pick different provider IDs (`copilot_cli`, `Codex`, `codex`, `cursor`)
...
- `providers` - Agent runtimes (copilot_cli, Codex, codex, cursor)
...
- `Codex.driver.ts` — Codex CLI via `@anthropic-ai/Codex-agent-sdk`
...
- **Settings Routing**: `/settings?section={sectionName}` — query parameter controls active tab (general, connections, Codex, copilot, codex, codex-plugins, projects, git, dashboard, etc.)
...
- `parseIcon()` returns typed icons: `emoji`, `icon`, `copilot-animate`, `Codex-animate`, `codex-animate`
- Animated icon types get special hover behavior in `space-selector.tsx`:
  ...
  - `Codex-animate` — geometric arm animation (individual `<line>` elements with `scaleX` CSS animations)
  - `codex-animate` — spin animation via `animate-spin-slow` CSS class
- SVG icons support custom props: `Bolt` has `animated?: boolean` (flame effects + gradient fill), `Codex` has `animate?: boolean` (arm retract/extend)
...
- Shared toolbar across Codex/copilot/codex variants, feature-gated by `variant` prop
