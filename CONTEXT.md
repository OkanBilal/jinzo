...
The per-driver lifecycle module in `codex-run-coordinator.ts`, created through `createCodexRunCoordinator(...)`. It exclusively owns live run state, in-memory run→thread identity, thread/subagent routing, serialized notification queues, server-request ownership, exactly-once turn finalization, usage flushing, and abort/timeout/cleanup/shutdown semantics. Its interface lets the **Codex session acquisition module** register or attach threads, while `codex.driver.ts` executes prepared `CodexRunSession` values, cleans up runs, and installs routing on a live **Codex app-server client**. Timeout and active abort both send `turn/interrupt` before finalizing; an already-aborted signal never starts a turn. Every coordinator instance owns independent maps so an invalidated/old provider adapter can finish without mutating its replacement.
