...
The per-driver lifecycle module in `codex-run-coordinator.ts`, created through `createCodexRunCoordinator(...)`. It exclusively owns live run state, in-memory run→thread identity, thread/subagent routing, serialized notification queues, server-request ownership, exactly-once turn finalization, usage flushing, and abort/timeout/cleanup/shutdown semantics. Its interface lets `codex.driver.ts` register or attach threads, execute a prepared `CodexRunSession`, clean up a run, and install routing on a live **Codex app-server client**. Timeout and active abort both send `turn/interrupt` before finalizing; an already-aborted signal never starts a turn. Every coordinator instance owns independent maps so an invalidated/old provider adapter can finish without mutating its replacement.
_Avoid_: module-global Codex run/session maps; routing notifications or server requests inside `codex.driver.ts`; resolving a run before its interrupt request has been dispatched; treating a subagent `turn/completed` as parent completion.

**Codex capability module**:
The deep discovery/management module in `codex-capabilities.ts`, created through `createCodexCapabilities(...)`. It owns Codex model, account, rate-limit, skill, and plugin operations; maps their app-server responses into shared adapter types; and hides plugin feature gating, TTL/in-flight cache deduplication, marketplace reference indexing, mutation invalidation, and connector-directory enrichment behind a small object interface. `codex.driver.ts` supplies the live app-server and CLI-health dependencies, delegates `ProviderDriver` capability methods to this module, and retains run/session lifecycle.
...
**Codex event mapper**:
The stateful projection module in `codex-event-mapper.ts`, created through `createCodexEventMapper(...)`. It owns notification/item interpretation, parent-vs-sub-thread filtering, agent/plan/command buffers, shell-command classification, tool-call event construction, context/usage snapshot deduplication, generated image/document discovery, and projection-state initialization. Its primary interface is `mapNotification(...)` → `WorkRunEvent[]`; review persistence stays outside through `onReviewCompleted`, while the **Codex run coordinator** sequences server notifications and run completion.
